<?php
	class Edit_wedding_model extends CI_Model
	{
	 function update(){	   
	...
	$query=$this->db->get_where('weddings',array('id'=> $wId));
			...
			{   if($row->photo != '../uploads/default.jpg' && ($path['file_name'] != "" && $path['file_name'] != null)){
...
create_square_image('uploads/'.$path['file_name'],'uploads/'.$path['file_name'],240);	
	...
	$get_month=array('decembrie','ianuarie','februarie','martie','aprilie','mai','iunie','iulie','august','septembrie','octombrie','decembrie');
	$v['userId']=id(); 
...
//	$v['numeF']=$this->input->post('numeF');
	...
	$v['scolor']=$this->input->post('scolor'); 
	    if ($path['file_name'] != "" && $path['file_name'] != null)
	     $v['photo']=$finalpath;
	$this->db->update('weddings',$v,array('id'=>$wId));   
